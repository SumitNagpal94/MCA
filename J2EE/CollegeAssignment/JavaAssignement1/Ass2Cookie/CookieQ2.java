...
class CookieQ2 extends HttpServlet
{
		...
		try
			...
			for (int i = 0; i < mac.length; i++)
				sb.append(String.format("%02X%s", mac[i], (i < mac.length - 1) ? "-" : ""));
                        ...
                        PrintWriter out = response.getWriter();
                        response.setContentType("text/html");
                        out.println("<html><body><label>UserName: "+request.getParameter("id")+"</label><label>Last Login: "+request.getParameter("dateTime")+"</label><label>Previous IPAddress: "+request.getParameter("ipAddress")+"</label><label>Previous MAC: "+request.getParameter("MAC")+"</label></br></body></html>");
